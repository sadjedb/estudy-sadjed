...
class ProjectService {
    ...
    async addOrUpdateProject(project) {
        const connection = await getDatabaseConnection();
        const { id, title, description, department, available_spots, supervisor } = project;

        if (id) {
            const query = `UPDATE projects SET title = ?, description = ?, department = ?, available_spots = ?, supervisor = ? WHERE id = ?`;
            const [result] = await connection.execute(query, [title, description, department, available_spots, supervisor, id]);
            ...
            const [result] = await connection.execute(query, [title, description, department, available_spots, supervisor]);

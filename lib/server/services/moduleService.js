...
class ModuleService {
    ...
    async addOrUpdateModule(module) {
        ...
        const { id, title, code, instructor, department, credits, description, schedule, location, syllabus = [] } = module;
        let moduleId = id;
        if (id) {
            // Update module
            const query = `UPDATE modules SET title = ?, code = ?, instructor = ?, department = ?, credits = ?, description = ?, schedule = ?, location = ? WHERE id = ?`;
            await connection.execute(query, [title, code, instructor, department, credits, description, schedule, location, id]);
            // Remove old syllabus
            await connection.execute('DELETE FROM module_syllabus WHERE module_id = ?', [id]);
        } else {
            // Insert new module
            const query = `INSERT INTO modules (title, code, instructor, department, credits, description, schedule, location, id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, UUID())`;
            await connection.execute(query, [title, code, instructor, department, credits, description, schedule, location]);
            // Get the inserted id
            const [rows] = await connection.execute('SELECT id FROM modules WHERE code = ? ORDER BY id DESC LIMIT 1', [code]);
            moduleId = rows[0].id;
        }
        // Insert syllabus
        for (const item of syllabus) {
            await connection.execute(
                ...
                [moduleId, item.week, item.description, item.courseFile || null, item.tdFile || null]
            );
        }
        return moduleId;
    }
    ...
    async removeModule(moduleId) {
        ...
        await connection.execute('DELETE FROM module_syllabus WHERE module_id = ?', [moduleId]);
        const [result] = await connection.execute('DELETE FROM modules WHERE id = ?', [moduleId]);
        return result.affectedRows > 0;
    }
}

...
const Page = ({ session }) => {
  ...
  const { data: projectsData, loading: loadingProjects } = useProject();
  const { data: wishlist, loading: loadingWishlist, addToWhishlist, removeFromWhishlist } = useWhishlist(session?.user?.id);
  ...
  const handleRemoveFromWishlist = async (projectId) => {
    if (!session) {
      alert("Please log in to remove projects from your wishlist.");
      return;
    }

    try {
      await removeFromWhishlist(projectId);
      alert("Project removed from wishlist!");
    } catch (error) {
      console.error("Wishlist error:", error);
      alert("Failed to remove project from wishlist.");
    }
  }
        ...
        {filteredProjects.length > 0 ? (
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredProjects.map((project) => {
              const isInWishlist = Array.isArray(wishlist?.wishlist)
                ? wishlist.wishlist.map(item => item.project_id || item.id).includes(project.id)
                : false;
              return (
                <div
                  key={project.id}
